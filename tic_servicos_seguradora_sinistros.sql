...
CREATE TABLE `sinistros` (
  `sinistro_id` int NOT NULL AUTO_INCREMENT,
  `apolice_id` int NOT NULL,
  `data_sinistro` date NOT NULL,
  `date_registro` datetime DEFAULT CURRENT_TIMESTAMP,
  `descricao` text NOT NULL,
  `valor_reclamado` decimal(15,2) DEFAULT NULL,
  `status` enum('Aberto','Em Ánalise','Aprovado','Rejeitado','Indenizado') DEFAULT 'Aberto',
  PRIMARY KEY (`sinistro_id`),
  KEY `apolice_id` (`apolice_id`),
  CONSTRAINT `sinistros_ibfk_1` FOREIGN KEY (`apolice_id`) REFERENCES `apolices` (`apolice_id`)
